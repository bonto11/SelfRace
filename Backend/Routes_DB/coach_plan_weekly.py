...
def db_insert_weekly_rows(rows: List[Dict[str, Any]]) -> int:
    ...
    try:
        res = supabase.table("coach_plan_weekly").insert(rows).execute()
        data = res.data or []
...
def db_clear_weekly_for_user_plan(user_id: int, plan_id: str) -> int:
    """
    DELETE všetkých weekly riadkov daného plánu pre usera.
    """
    try:
        res = (
            supabase.table("coach_plan_weekly")
            .delete()
            .eq("user_id", user_id)
            .eq("plan_id", plan_id)
            .execute()
...
def db_get_weekly_for_user_plan(user_id: int, plan_id: str) -> List[Dict[str, Any]]:
    """
...
def db_get_latest_plan_id_for_user(user_id: int) -> Optional[str]:
    """
