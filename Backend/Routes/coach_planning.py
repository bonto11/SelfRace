...
def _norm_goal(goal_block: Any, goal_kind: Optional[str] = None, race_goal: Optional[str] = None) -> str:
  """
  Zjednotená normalizácia cieľa:
  - podporuje starý tvar {"kind": "...", "race_goal": "..."}
  - aj nový tvar {"goal_kind": "..."}
  - aj plain string
  """
  # nový/štandardný tvar – goal_kind
  if isinstance(goal_block, dict):
    kind = (goal_block.get("kind") or goal_block.get("goal_kind") or goal_kind or "").strip()
    rg = goal_block.get("race_goal") or race_goal
    if kind == "race_time" and rg:
      return f"race_time:{rg}"
    return kind or "improve_overall"

  if isinstance(goal_block, str):
    return goal_block or "improve_overall"

  # fallback len z goal_kind/race_goal
  if goal_kind:
    if goal_kind == "race_time" and race_goal:
      return f"race_time:{race_goal}"
    return goal_kind

  return "improve_overall"


def _normalize_payload(payload: dict) -> dict:
  """
  Zoberie aktuálny payload z FE (nový kontrakt) + prípadný legacy goal_structured
  a vytvorí jeden zmysluplný objekt pre AI.
  """
  ...
  # weeks – preferuj explicitné, potom goal_block / goal_struct
  weeks = int(
    payload.get("weeks")
  ...
  # goal kind + race goal (ak by si niekde doplnil)
  goal_kind = (
    payload.get("goal_kind")
    or goal_block.get("goal_kind")
    or goal_struct.get("goal_kind")
  )
  race_goal = (
    payload.get("race_goal")
    or goal_block.get("race_goal")
    or goal_struct.get("race_goal")
  )
  goal_str = _norm_goal(goal_block or goal_struct, goal_kind=goal_kind, race_goal=race_goal)

  # primary sports – skúsiť flat, potom goal_structured, potom sports_block
  primary_sports = (
    payload.get("primary_sports")
    or goal_struct.get("primary_sports")
  )
  if not primary_sports:
    main = sports_block.get("main_sport")
    secondary = [
      x.get("sport")
      for x in (sports_block.get("secondary_mix") or [])
      if (x or {}).get("share_pct", 0) > 0
    ]
    tmp = [s for s in [main, *secondary] if s]
    primary_sports = tmp or ["run", "ride", "strength"]

  # persona – ak ju časom budeš mať, preferuj flat/pod goal
  ...
  externals: List[dict] = (
    payload.get("externals")
    or goal_struct.get("external_activities")
    or []
  )

  injuries: List[dict] = (
    payload.get("injuries")
    or goal_struct.get("injuries")
    or []
  )
  ...
  # plan_start_date – preferuj plan_start_date, potom goal.start_date, potom legacy
  plan_start_date = (
  ...
  # intensity model – preferuj flat, inak derive z legacy flagov
...
def coach_analyze(user_id: int, request: Request, payload: dict = Body(...)):
  try:
    ...
    # generate_plan_json robí vnútorné fallbacky, tu stačí prvý model – ale necháme cyklus pre budúcnosť
    for m in models:
