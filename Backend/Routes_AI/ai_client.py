...
def parse_ai_json(raw: str) -> Tuple[Optional[dict], str, str]:
    """
    Return (parsed_or_None, cleaned_text, raw_text).
    Nikdy neháče – caller si rozhodne, čo s tým.
    """
    if not raw:
        return None, "", ""
    try:
...
def _llm_models_priority(explicit_model: Optional[str]) -> List[str]:
    """
    Poradie modelov:
      - ak zadáš explicitný → skúsi sa ako prvý
      - potom env OPENAI_MODEL_FALLBACKS
...
def call_json_model(
    *,
    ...
    """
    Univerzálny JSON LLM caller.

    - context_payload: dict → pošle sa do promptu ako JSON
    - system_prompt: systémová rola (štýl / pravidlá)
    - user_instructions: text, ktorý vysvetľuje úlohu + schému
    - model: preferovaný model (inak fallback zo zoznamu)
    - max_tokens: horný limit na odpoveď
    - debug_raw: ak True, vráti 2. hodnotu s trace/raw

    Vráti (parsed_json, debug_trace). Pri chybe hádže HTTPException(500).
    """
    ...
    # user prompt = instructions + context JSON
    user_txt = (
    ...
    # ak sem spadneš, všetko zlyhalo → HTTP 500 pre FE
    detail = f"AI analyze failed: {last_err or 'unknown error'}"
    if debug_raw:
        trace["raw"] = last_raw
        trace["cleaned"] = last_cleaned
        raise HTTPException(
            status_code=500,
            detail={"error": detail, "trace": trace},
        )
