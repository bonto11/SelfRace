    ...
    for m in models:
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                trace["attempts"].append({"model": m, "attempt": attempt, "tokens_budget": budget, "duration_ms": dur_ms, "ok": True, "raw_preview": (raw[:800] + ("…[truncated]" if len(raw) > 800 else ""))})
                ...
                trace["attempts"].append({"model": m, "attempt": attempt, "tokens_budget": budget, "duration_ms": dur_ms, "ok": False, "error": last_err})
