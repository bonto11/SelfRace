...
def build_narrative(ctx: dict, weeks: int) -> dict:
    ...
    period = []
    if prev: period.append(f"Za posledných {min(len(prev)+1, weeks)} týždňov držíš beh ~{km_prev:.1f} km/týž. a TRIMP ~{tr_prev:.0f}.")
    if hrv14 or rhr14: period.append(f"Recovery trend: HRV ~{hrv14:.0f} ms, RHR ~{rhr14:.0f} bpm (predposledné 2 týždne).")
    ...
    if hrv7 or rhr7: last_week.append(f"HRV {hrv7:.0f} ms ({hrv7-hrv14:+.0f} vs. predchádzajúce 2 týždne), RHR {rhr7:.1f} bpm ({rhr7-rhr14:+.1f}).")
    ...
    return {"period_summary":" ".join(period) or None, "last_week_summary":" ".join(last_week) or None}