...
def _canonical_sport(s: Any) -> str:
    ...
    v = str(s).lower()
    if v.startswith("run") or "run" in v or v in ("trail", "trail_run"): return "run"
    if v.startswith("ride") or v.startswith("cycle") or v.startswith("bike"): return "ride"
    if v.startswith("str") or "strength" in v or "gym" in v or "weights" in v: return "strength"
...
def _bests_dates_to_days_ago(bests: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(bests, dict): return bests
    out = dict(bests)
    for sport_key in ("run", "ride"):
        ...
        for it in items:
            ...
            if d is not None: it2["days_ago"] = d
            it2.pop("date", None)
...
def _load_user_profile_for_analysis(user_id: int, ctx: AuthCtx) -> Dict[str, Any]:
    ...
    return {"id": user_id, "sex": stat.get("sex"), "age": age, "height_cm": stat.get("height_cm"), "weight_kg": weight_kg}

def _minify_external_events_for_ai(ext: Any) -> Any:
    if not isinstance(ext, dict): return ext
    ...
    for e in events:
        ...
        cleaned_events.append({
            "days_from_today": _days_from_today(dt), "weekday": e.get("weekday"), "sport": e.get("sport"),
            "duration_min": e.get("duration_min"), "priority": e.get("priority"), "title": e.get("title"),
        })
    win = ext.get("window")
    if isinstance(win, dict):
        out["window"] = {"from_days_from_today": _days_from_today(win.get("from")), "to_days_from_today": _days_from_today(win.get("to")), "events": cleaned_events}
    else: out["events"] = cleaned_events
    return out

# ✂️ OSEKANIE SEGMENTOV: Len 8 najdôležitejších vzoriek!
def _get_minified_segments(user_id: int, activity_id: int, ctx: AuthCtx) -> List[Dict[str, Any]]:
    segments = []
    laps = db_get_activity_laps(user_id, activity_id, ctx=ctx) or []
    if len(laps) < 2:
        laps = db_get_activity_splits(user_id, activity_id, ctx=ctx) or []
    
    if len(laps) < 2:
        return []

    # Smart picker - maximálne 8 bodov
    max_items = 8
    total = len(laps)
    if total <= max_items:
        ...
        idxs_set = {0, total - 1}
        step = (total - 1) / (max_items - 1)
        for i in range(1, max_items - 1):
            idxs_set.add(int(round(i * step)))
        idxs = sorted(list(idxs_set))
...
def build_last_activities_block_for_analysis(user_id: int, *, ctx: AuthCtx, limit: int = 6) -> List[Dict[str, Any]]:
    if limit <= 0: limit = 4
    since_iso = (datetime.now(timezone.utc) - timedelta(days=60)).date().isoformat()
    ids = db_get_recent_activity_ids(user_id=user_id, since_iso_date=since_iso, limit=limit, ctx=ctx)
    if not ids: return []
    summary_rows = db_get_summary_for_activities(user_id=user_id, activity_ids=ids, ctx=ctx) or []
    if not summary_rows: return []
    enr_rows = db_get_enrichment_for_activities(user_id=user_id, activity_ids=ids, ctx=ctx) or []
    enr_by_id: Dict[int, Dict[str, Any]] = {aid: r for r in enr_rows if (aid := _to_int(r.get("activity_id"))) is not None}

    def _date_key(row: Dict[str, Any]) -> str: return str(row.get("date") or "")[:19]
    ...
    for r in sorted(summary_rows, key=_date_key, reverse=True):
        ...
        avg_pace_s = int(moving_s / dist_km) if (dist_km and dist_km > 0 and moving_s and moving_s > 0) else None
        ...
        intensity = "easy"
        if z45 > 5: intensity = "hard"
        elif z45 > 0 or (dur_min and z12 < (dur_min * 0.8)): intensity = "moderate"

        act_obj = {
            "date": _rel_day_label(date_str), "sport": sport, "duration_min": round(dur_min) if dur_min else None,
            "distance_km": round(dist_km, 2) if dist_km else None, "avg_pace_s": avg_pace_s, "avg_hr": avg_hr, "intensity": intensity,
        }

        if sport in ["run", "ride"]:
            segments = _get_minified_segments(user_id, aid, ctx)
            if segments: act_obj["segments"] = segments
...
def build_base_input(user_id: int) -> Dict[str, Any]:
    return {
        "schema_version": 1,
        "user": {"id": user_id, "sex": None, "age": None, "height_cm": None, "weight_kg": None, "training_age_years": None},
        "prefs": {"goal_kind": None, "weeks": None, "plan_start_date": None, "main_sport": None, "secondary_mix": [], "strength_settings": None, "weekly_time_budget_min": None, "hard_days_per_week_max": None, "notes_for_coach": None},
        ...
        "external_events": None, "last_activities": [], "latest_paces": None, "is_returning_beginner": False,
    }
...
def build_input_from_db(user_id: int, *, ctx: AuthCtx) -> Dict[str, Any]:
    ...
    input_data["bests"] = _bests_dates_to_days_ago(service_build_bests_block_for_analysis(user_id, ctx=ctx) or {})
    input_data["recent_load"] = service_build_recent_load_block_for_analysis(user_id=user_id, window_days=42, ctx=ctx)
    ...
    input_data["external_events"] = _minify_external_events_for_ai(service_build_external_events_block_for_analysis(user_id=user_id, ctx=ctx))
