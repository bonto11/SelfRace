...
def _build_activity_block_from_rows(
    *,
    ...
    # "hard" if meaningful time in Z4+Z5 (minutes)
...
def build_input_from_db(
    ...
    *,
    ...
    """
    Minimal activity review input:
      - context.recovery (HRV/RHR/sleep/trend)
      - context.recent_load (week horizon only)
      - activity metrics + zones (minutes)

    service=False -> RLS, requires JWT
    service=True  -> service client, JWT not needed
    """
    jwt = None if service else require_jwt(user_jwt)

    input_data = build_base_input(user_id, activity_id)

    # --- context blocks (ONLY what impacts 1-week evaluation) ---
    ...
    # Use 14d window to compute stable weekly blocks, but we minify to (-2..0)
    recent_load = service_build_recent_load_block_for_analysis(
        user_id=user_id,
        window_days=14,
        user_jwt=jwt,
        service=service,
    )
    input_data["context"]["recent_load"] = _minify_recent_load_to_week_horizon(recent_load)
    ...
    if not isinstance(summary_row, dict):
        # Keep base shape; upstream will decide whether to call AI or return error.
        return input_data
