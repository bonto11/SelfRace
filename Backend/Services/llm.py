...
def coerce_to_schema(obj: dict) -> dict:
    if not isinstance(obj, dict): raise ValueError("LLM output is not a JSON object")
    out: Dict[str, Any] = {
    ...
    if isinstance(nwp, list) and all(isinstance(x,str) for x in nwp):
        out["_meta"]["coerced"] = True; out["_meta"]["plan_source"] = "coerced_from_guidelines"
        out["next_week_plan"] = None; out["_guidelines"] = nwp
    elif isinstance(nwp, dict):
        out["next_week_plan"] = nwp
    else:
        out["_meta"]["coerced"] = True; out["_meta"]["plan_source"] = "coerced_empty"
        out["next_week_plan"] = None
    return out

def enforce_minimum_plan(parsed: dict, llm_input: dict, build_min_plan_fn) -> dict:
    ...
    if not plan or not has_any:
        parsed.setdefault("summary", "Auto-filled plan based on your recent context (guidelines detected).")
        parsed["next_week_plan"] = build_min_plan_fn(llm_input)
        ...
        parsed.setdefault("_meta", {})["plan_source"] = parsed.get("_meta", {}).get("plan_source","llm")
    return parsed

def try_llm_call(payload_json: dict, model: str) -> dict:
    ...
    Constraints: Return ONLY JSON. Prefer full 7-day plan. Use thresholds/zones if present. Keep progression ≤10%.
    """
    ...
    if not raw: raise RuntimeError("empty_response_chat_json_object")
    try: parsed = json.loads(raw)
    except Exception: parsed = extract_json_block(raw)
