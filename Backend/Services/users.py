...
def service_resolve_user(auth_uid: str) -> Optional[int]:
    """
    Resolve /users/resolve – nájde user_id podľa auth_uid.
    """
    row = db_get_user_by_auth_uid(auth_uid)
    if not row:
        return None
    return int(row["id"])


def service_get_user_uid(user_id: int) -> str:
    """
    Vráti auth_uid pre dané user_id, alebo hodí RuntimeError ak chýba.
    (Kompatibilita s pôvodným get_user_uid.)
    """
    uid = db_get_user_uid(user_id)
    if not uid:
...
def service_create_user(
    ...
    """
    existing = db_get_user_by_email(mail_address)
    if existing:
    ...
    row = db_insert_user(payload)
    print("Úspešne vložené:", row)
    return row


def service_get_user_by_email(mail_address: str) -> Optional[Dict[str, Any]]:
    """
    Wrapper na get by email.
    """
    return db_get_user_by_email(mail_address)


def service_update_user(mail_address: str, **fields: Any) -> Optional[Dict[str, Any]]:
    """
    Update podľa mail_address.
    """
    if not fields:
        return None
    return db_update_user_by_email(mail_address, fields)


def service_delete_user(mail_address: str) -> None:
    """
    Delete podľa mail_address.
    """
    db_delete_user_by_email(mail_address)
