...
def db_link_session_to_activity(
    user_id: int, *, ctx: AuthCtx, id: int, activity_id: Optional[int]
...
def db_get_daily_session_by_id(
    user_id: int, id: int, *, ctx: AuthCtx
) -> Optional[Dict[str, Any]]:
    ...
    try:
        res = (
            sb.table(TABLE_COACH_PLAN_DAILY)
            .select("id,user_id,plan_date,session_index")
            .eq("id", int(id))
            .eq("user_id", int(user_id))
            .limit(1)
            .execute()
...
def db_update_daily_session_date(
    user_id: int, id: int, *, plan_date: str, session_index: int, ctx: AuthCtx
) -> Optional[Dict[str, Any]]:
    # Nezmenené - jednoznačné cez (user_id, id)
    ...
    for m in moves:
        try:
            ...
            rest_row = db_get_rest_session_on_day(
                user_id=user_id, plan_meta_id=plan_meta_id, plan_date=to_date, ctx=ctx
            )
            if rest_row and rest_row.get("id") is not None:
                deleted = db_delete_daily_session(
                    user_id=user_id, session_id=int(rest_row["id"]), ctx=ctx
                )
                if not deleted:
...
def db_check_daily_data_exists(
    user_id: int, plan_meta_id: Optional[int], *, ctx: AuthCtx
) -> bool:
...
def db_update_daily_session_data(
    user_id: int, session_id: int, update_data: Dict[str, Any], *, ctx: AuthCtx
) -> Optional[Dict[str, Any]]:
    # Nezmenené - jednoznačné cez (user_id, id)
    ...
    try:
        ...
        if (
            "activity_id" in payload
            and payload["activity_id"] is None
...
def db_get_unmatched_activities(
    ...
    *,
    ...
    try:
        ...
        matched_ids = {
            int(r["activity_id"])
            for r in (plan_res.data or [])
            if r.get("activity_id") is not None
        }

        activities = db_get_activities_recent(
            ctx=ctx, user_id=user_id, since_iso_date=since_iso
        )

        unmatched = [a for a in activities if int(a["activity_id"]) not in matched_ids]
        return unmatched
    except Exception as e:
...
def db_get_daily_session_by_id_full(
    user_id: int, id: int, *, ctx: AuthCtx
) -> Optional[Dict[str, Any]]:
    # Nezmenené - jednoznačné cez (user_id, id)
...
def db_append_preview_thread_entry(
    user_id: int, id: int, entry: Dict[str, Any], *, ctx: AuthCtx
) -> bool:
    ...
    try:
        ...
        thread.append(entry)
        sb.table(TABLE_COACH_PLAN_DAILY).update(
            {"preview_thread": thread, "updated_at": _now_iso()}
        ).eq("id", int(id)).eq("user_id", int(user_id)).execute()
        return True
    except Exception as e:
...
def db_apply_session_preview_update(
    ...
    *,
    ...
    try:
        sb.table(TABLE_COACH_PLAN_DAILY).update(payload).eq("id", int(id)).eq(
            "user_id", int(user_id)
        ).execute()
        return True
    except Exception as e:
...
def db_get_last_planned_daily_session_for_user(
    ...
    """
    sb = get_sb(
        ctx, caller="coach_plan_daily.db_get_last_planned_daily_session_for_user"
    )
    try:
