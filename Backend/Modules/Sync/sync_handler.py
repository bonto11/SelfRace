    ...
    while True:
        try:
            ...
            if (
                ...
                wait = (
                    15 * 60 if tries == max_retries - 1 else 60 * (2**tries)
                )  # 1m, 2m, ... 15m
...
def sync_activities(
    user_id: int, force_full_30d: bool = False, archive_raw: bool = False
) -> int:
    ...
    print(
        f"➡️  Sťahujem aktivity po {download_since_utc.isoformat()} (epoch={after_epoch})"
    )
    ...
    for act in reversed(activities):
        ...
        if fetched_full >= MAX_FULL_DETAILS_PER_RUN:
            print(
                f"⏸️  Limit FULL fetchov dosiahnutý ({MAX_FULL_DETAILS_PER_RUN}). Zvyšok nabudúce."
            )
            break
        ...
        try:
            ...
            if sport != "run":
                ...
                if ok_summary:
                    saved_summary += 1
                    print(
                        f"💾 {saved_summary:03d}  Uložené: {full.get('name')} [{full.get('start_date_local')}], id={activity_id} • {sport} (bez laps/splits)"
                    )
                continue
            ...
            if mode == "laps" and decision.get("laps"):
                try:
                    ...
                    pass
                inserted = sql_dm.replace_activity_laps(
                    user_id, activity_id, decision["laps"]
                )
            else:
                try:
                    ...
                    pass
                inserted = sql_dm.replace_activity_splits(
                    user_id, activity_id, decision.get("splits") or []
                )
            ...
            if ok_summary:
                ...
                print(
                    f"⚠️  Preskočené (summary zlyhalo): id={activity_id}  {full.get('name')}"
                )
    ...
    for act in reversed(headers):
        ...
        if sport != "run":
            if ok_summary:
                saved += 1
                print(
                    f"💾 {saved:03d}  Uložené: {full.get('name')} [{full.get('start_date_local')}] • {sport} (bez laps/splits)"
                )
            continue
        ...
        if mode == "laps" and decision.get("laps"):
            ...
            sql_dm.replace_activity_splits(
                user_id, activity_id, decision.get("splits") or []
            )
            rows_flag = True
        ...
        if ok_summary:
            saved += 1
            print(
                f"💾 {saved:03d}  Uložené: {full.get('name')}  [{full.get('start_date_local')}]  id={activity_id}  • {mode or sport}  (rows={rows_flag})"
            )
        else:
            print(
                f"⚠️ Preskočené (summary zlyhalo): id={activity_id} {full.get('name')}"
...
def _save_history_checkpoint(
    payload: dict, path: str = "data/history_checkpoint.json"
) -> None:
...
def sync_history(
    ...
    assert (
        from_date.tzinfo is not None and to_date.tzinfo is not None
    ), "Použi timezone-aware UTC dátumy"
    ...
    while current < to_date:
        ...
        print(f"\n========== SYNC {current:%Y-%m} ==========")
        print(
            f"=== Window {current:%Y-%m-%d} → {window_end:%Y-%m-%d} (epoch after={int(current.timestamp())}) ==="
        )

        while True:
            try:
                saved = sync_activities_for_window(
                    user_id, current, window_end, archive_raw=archive_raw
                )
                total_saved += saved
                if use_checkpoint:
                ...
                if is_429(e):
                    ...
                    print(
                        f"⏳ 429 Too Many Requests. Čakám {wait//60} min a skúšam ZNOVU tento mesiac …"
                    )
                    time.sleep(wait)
                    continue
                else:
...
def cache_streams_for_activity(
    ...
    ok = sql_dm.replace_activity_details(
        user_id, activity_id, streams, activity_date=activity_date
    )
    if ok:
        print(
            f"✅ activity_details nahradené (user_id={user_id}, activity_id={activity_id})"
        )
    else:
