...
export default function ActivityTable({ start, end }: { start?: string; end?: string }) {
  ...
  const headerTitle =
    start && end ? `História (${start} → ${end})` : "História (vyber týždeň v grafe vyššie)";
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded shadow space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        >
          {syncing && <span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></span>}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse text-center">
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={7} className="py-6 opacity-70">Načítavam…</td></tr>
            )}
            {!loading && rows.map((row) => (
              ...
              >
                ...
                <td>{row.moving_time_s ? `${Math.floor(row.moving_time_s / 60)} min` : "-"}</td>
                <td>{row.average_heartrate_bpm ?? "-"}</td>
                <td>{row.max_heartrate_bpm ?? "-"}</td>
                <td>{row.distance_m ? `${(row.distance_m / 1000).toFixed(2)} km` : "-"}</td>
              </tr>
            ))}
            {!loading && !rows.length && (
              <tr>
                <td colSpan={7} className="py-6 opacity-70">
                  {start && end ? "Žiadne aktivity v tomto období." : "Klikni na týždeň v grafe, aby sa zobrazili aktivity."}
                </td>
              </tr>
            )}
          </tbody>
      ...
      {selected && (
        <div className="bg-gray-100 dark:bg-gray-900 p-4 rounded shadow">
          <h3 className="font-bold mb-2">
            {selected.summary.name} ({new Date(selected.summary.date).toLocaleDateString("sk-SK")})
          </h3>

          <p>Sport: {selected.summary.sport_type}</p>
          <p>Distance: {selected.summary.distance_m ? (selected.summary.distance_m / 1000).toFixed(2) + " km" : "-"}</p>
          <p>Moving time: {selected.summary.moving_time_s ? Math.floor(selected.summary.moving_time_s / 60) + " min" : "-"}</p>
            ...
            <>
              ...
              <ul className="list-disc pl-5">
                {selected.laps.map((lap, idx) => (
                  <li key={lap.lap_index ?? idx}>Lap {lap.lap_index}: {lap.distance_m} m, {lap.moving_time_s}s</li>
                ))}
              </ul>
            </>
          )}
            ...
            <>
              ...
              <ul className="list-disc pl-5">
                {selected.splits.map((split, idx) => (
                  <li key={split.split_index ?? idx}>Split {split.split_index}: {split.distance_m} m, {split.moving_time_s}s</li>
                ))}
              </ul>
            </>
          )}
