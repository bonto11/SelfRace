...
import {
  ...
  Title, Tooltip, Legend, ChartData, ChartOptions,
} from "chart.js";
...
type WeekRow = {
  ...
  km_run: number; km_ride: number; km_total: number;
  time_min: number; time_run_min: number; time_ride_min: number; time_strength_min: number; time_other_min: number;
  trimp_run: number; trimp_bike: number; trimp_strength: number; trimp_other: number; trimp: number;
  monotony: { km?: number; time?: number; trimp?: number };
  strain:   { km?: number; time?: number; trimp?: number };
};

const C = {
  run: "#22D3EE", bike: "#A78BFA", strength: "#F59E0B", other: "#9CA3AF",
...
export default function TrendWeeklyLoad({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.table(norm.map((w) => ({
          week: w.week, km: w.km_total, time_min: w.time_min, trimp: w.trimp,
          mono_km: w.monotony.km, mono_time: w.monotony.time, mono_trimp: w.monotony.trimp,
          strain_km: w.strain.km, strain_time: w.strain.time, strain_trimp: w.strain.trimp,
        })));
  ...
  const monoSeries = useMemo(() => weeks.map((w) => (w.monotony?.[metric] ?? null)), [weeks, metric]);
  const strainSeries = useMemo(() => weeks.map((w) => (w.strain?.[metric] ?? null)), [weeks, metric]);

  const monoMax = monoSeries.filter((v): v is number => v != null).length ? Math.max(1, ...monoSeries.filter((v): v is number => v != null)) : 3;
  const strainMax = strainSeries.filter((v): v is number => v != null).length ? Math.max(1, ...strainSeries.filter((v): v is number => v != null)) : 10;
  ...
  const datasets = useMemo(() => {
    ...
    if (metric === "km") {
      if (sRun)  arr.push({ type: "bar" as const, label: "Km (run)",  data: weeks.map((w) => w.km_run),  backgroundColor: alpha(C.run, 0.85),  borderColor: C.run,  borderWidth: 1, yAxisID: "y" });
      if (sBike) arr.push({ type: "bar" as const, label: "Km (bike)", data: weeks.map((w) => w.km_ride), backgroundColor: alpha(C.bike, 0.85), borderColor: C.bike, borderWidth: 1, yAxisID: "y" });
    }

    if (metric === "time") {
      if (sRun)      arr.push({ type: "bar" as const, label: "Run",      data: weeks.map((w) => w.time_run_min),      backgroundColor: alpha(C.run, 0.85),      borderColor: C.run,      borderWidth: 1, yAxisID: "y" });
      if (sBike)     arr.push({ type: "bar" as const, label: "Bike",     data: weeks.map((w) => w.time_ride_min),     backgroundColor: alpha(C.bike, 0.85),     borderColor: C.bike,     borderWidth: 1, yAxisID: "y" });
      if (sStrength) arr.push({ type: "bar" as const, label: "Strength", data: weeks.map((w) => w.time_strength_min), backgroundColor: alpha(C.strength, 0.9),   borderColor: C.strength, borderWidth: 1, yAxisID: "y" });
      if (sOther)    arr.push({ type: "bar" as const, label: "Other",    data: weeks.map((w) => w.time_other_min),    backgroundColor: alpha(C.other, 0.9),      borderColor: C.other,    borderWidth: 1, yAxisID: "y" });
    }

    if (metric === "trimp") {
      if (sRun)      arr.push({ type: "bar" as const, label: "TRIMP (run)",      data: weeks.map((w) => w.trimp_run),      backgroundColor: alpha(C.run, 0.85),      borderColor: C.run,      borderWidth: 1, yAxisID: "y" });
      if (sBike)     arr.push({ type: "bar" as const, label: "TRIMP (bike)",     data: weeks.map((w) => w.trimp_bike),     backgroundColor: alpha(C.bike, 0.85),     borderColor: C.bike,     borderWidth: 1, yAxisID: "y" });
      if (sStrength) arr.push({ type: "bar" as const, label: "TRIMP (strength)", data: weeks.map((w) => w.trimp_strength), backgroundColor: alpha(C.strength, 0.9),   borderColor: C.strength, borderWidth: 1, yAxisID: "y" });
      if (sOther)    arr.push({ type: "bar" as const, label: "TRIMP (other)",    data: weeks.map((w) => w.trimp_other),    backgroundColor: alpha(C.other, 0.9),      borderColor: C.other,    borderWidth: 1, yAxisID: "y" });
    }

    // Monotony (pravá os y1)
    arr.push({ type: "line" as const, label: "Monotony", data: monoSeries, yAxisID: "y1",
      borderColor: C.monotony, backgroundColor: C.monotony, tension: 0.3, pointRadius: 2, borderWidth: 3, spanGaps: true, order: 99 });

    // Strain (pravá os y2)
    arr.push({ type: "line" as const, label: "Strain", data: strainSeries, yAxisID: "y2",
  ...
  const options: ChartOptions<"bar" | "line"> = {
    ...
    scales: {
      y:  { beginAtZero: true, title: { display: true, text: metric === "km" ? "km" : metric === "time" ? "min" : "TRIMP" }, grid: { color: "rgba(255,255,255,0.07)" } },
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded shadow relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sRun} onChange={(e) => setSRun(e.target.checked)} /> Beh
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sBike} onChange={(e) => setSBike(e.target.checked)} /> Bike
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sStrength} onChange={(e) => setSStrength(e.target.checked)} /> Sila
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sOther} onChange={(e) => setSOther(e.target.checked)} /> Iné
            </label>
          </div>
          ...
          <div className="relative">
            <button
              ...
              onClick={() => setShowHelp((v) => !v)} aria-label="Help" title="Čo je zobrazené?">
              i
            </button>
            {showHelp && (
      ...
      <div style={{ height: 360 }}>
        {loading ? <div className="opacity-70 text-sm">Načítavam…</div>
