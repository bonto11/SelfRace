...
const SPORT_COLORS: Record<string, string> = {
  ...
  strength: THEME.chart.strength, mixed: THEME.chart.mixed, skate: THEME.chart.skate,
  walk: THEME.chart.walk, other: THEME.chart.other,
};

type DayCellData = {
  iso: string; inMonth: boolean; day: number | null;
...
function useMonthActivities(year:number, month0:number){
  ...
  React.useEffect(()=>{
    const total=42, offset=startWeekday(year,month0);
...
export default function ActivitiesCalendar({ year:yy, month:mm }:{ year?:number; month?:number }){
  ...
  const jump = (dir:-1|1)=>{
    const d=new Date(year,month0,1); d.setMonth(d.getMonth()+dir);
    setYear(d.getFullYear()); setMonth0(d.getMonth()); setSelectedIso(null);
  };

  const label=new Date(year,month0,1).toLocaleDateString("sk-SK", { month:"long", year:"numeric" });
  const niceDate=(s:string)=> new Date(s).toLocaleDateString("sk-SK",{ weekday:"short", day:"2-digit", month:"short" });
