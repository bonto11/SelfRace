...
import { isoDate, rollingMean, bandsAround, wrapTextToLines } from "@/shared/utils/recovery";
...
export default function DetailHRV() {
  ...
  const data = useMemo(()=>({
    labels: labelsISO,
    datasets: [
      { type:"line" as const, label:"Baseline −5%", data:lower, borderColor:"transparent", pointRadius:0, fill:"+1", backgroundColor:"rgba(34,197,94,0.18)" },
      { type:"line" as const, label:"Baseline (14d priemer)", data:baseline, borderColor:THEME.chart.monotony, backgroundColor:THEME.chart.monotony, borderWidth:2, pointRadius:0 },
      { type:"line" as const, label:"Baseline +5%", data:upper, borderColor:"transparent", pointRadius:0, fill:"-1", backgroundColor:"rgba(34,197,94,0.18)" },
      { type:"line" as const, label:"HRV (RMSSD)", data:hrv, borderColor:"#06b6d4", backgroundColor:"#06b6d4", pointRadius:3, tension:0.25 },
    ],
  ...
  const options = useMemo(()=>buildRecoveryLineOptions({
    ...
    tooltipLabelForItem:(ctx)=>{
      ...
      if (ctx.datasetIndex === 3) {
        const v = hrv[idx];
        const lines = [`HRV: ${isFinite(v as number) ? Math.round(v as number) : "—"} ms`];
        const c = comments.get(iso); if (c) lines.push(...wrapTextToLines(c,44));
        return lines;
      }
      if (ctx.datasetIndex === 1) {
        const b = baseline[idx];
        return [`Baseline: ${isFinite(b as number) ? Math.round(b as number) : "—"} ms`];
      }
      return "";
    },
