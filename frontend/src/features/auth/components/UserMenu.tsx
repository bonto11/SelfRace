...
export default function UserMenu({ user }: Props) {
  ...
  useEffect(() => {
    console.log('[UserMenu] init – reading user via sb.auth.getUser()');
    sb.auth.getUser().then(({ data }) => {
      const usr = data?.user as User | null;
      console.log('[UserMenu] getUser()', { hasUser: !!usr, id: usr?.id ?? null });
      if (!usr) return;
      const meta = (usr.user_metadata as Record<string, any>) || {};
      setU({
        email: usr.email ?? "",
    ...
    const sub = sb.auth.onAuthStateChange(
      (_ev: AuthChangeEvent, session: Session | null) => {
        console.log('[UserMenu] onAuthStateChange', {
          event: _ev,
          hasSession: !!session,
  ...
  async function signOut() {
    setBusy("signout");
    try {
      console.log('[UserMenu] signOut() → calling sb.auth.signOut()');
      await sb.auth.signOut(); // vyčistí klienta; middleware cookie zruší na ďalší request
      setOpen(false);
      router.replace("/signin");
    } catch (e: any) {
      console.error('[UserMenu] signOut ERROR', e?.message ?? e);
    } finally {
      setBusy(null);
    }
  }
  ...
  async function handlePasswordReset() {
    try {
      ...
      console.log('[UserMenu] resetPasswordForEmail() for', data.user.email);
      ...
      console.error('[UserMenu] handlePasswordReset ERROR', e?.message ?? e);
