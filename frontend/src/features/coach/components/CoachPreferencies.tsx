...
import type {
  CoachPrefs, GoalKind, SportKind, CoachPersona,
  ExternalActivity, ExternalIntensity, ExternalSport,
  Injury, InjuryArea, InjuryType, RehabFocus,
...
import { NO_X, SURFACE_INSET, SURFACE_INLINE, SECTION, FORM_GRID_TWO, FORM_GRID_SPLIT, PILL_BUTTON } from "@/shared/ui/classes";
...
const FOCUS_CHOICES = ["ankle_strength","foot_intrinsics","calf_strength","hamstrings","glutes","core_stability","thoracic_mobility","shoulder_stability"];
const AVOID_CHOICES = ["impact_high","downhill_runs","hard_surfaces","back_to_back_speed"];

const ACTIVE_PILL = "bg-emerald-600/90 border-emerald-500 text-white shadow-[inset_0_0_0_2px_rgba(16,185,129,.25)]";
...
function InfoPopover({ text }: { text: string }) {
  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen(v=>!v)} className="px-2 py-1 rounded-lg border border-white/15 bg-white/5 hover:bg-white/10 text-xs">i</button>
      {open && (
        <div className={[SURFACE_INSET, "absolute right-0 mt-2 w-[min(74vw,360px)] p-3 text-xs leading-snug z-30"].join(" ")}>{text}</div>
      )}
    </div>
  );
}
...
export default function PrefsForm() {
  ...
  const toggleInArray = <T,>(arr: T[] | undefined, v: T): T[] => (arr ?? []).includes(v) ? (arr ?? []).filter(x => x !== v) : [ ...(arr ?? []), v ];
    ...
    <div className={["space-y-4", NO_X].join(" ")}>
      ...
      <section className={SECTION}>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className={[PILL_BUTTON, !local.goal_kind ? ACTIVE_PILL : "border-white/15"].join(" ")}
          >
            None
          </button>
        </div>
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
          <TextField
            ...
            value={local.weeks ?? ""}
            onChange={(e) => setPref("weeks", (e.target as HTMLInputElement).value ? Number((e.target as HTMLInputElement).value) : undefined)}
            inputMode="numeric"
          />
          <TextField
      ...
      <section className={SECTION}>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-xs opacity-70">Presets lock sliders · Custom unlocks · None disables</div>
        </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (key === null) {
                    ...
                    setPref("coach_tone", local.coach_tone ?? { directness: 50, praise: 50, challenge: 50, emoji: 20, explain: 60 });
                  } else {
        ...
        <div className="mt-3 grid grid-cols-1 gap-2">
          {(["directness","praise","challenge","emoji","explain"] as const).map((key) => {
            const v = Number((tone as any)[key] ?? 50);
            const locked = local.coach_voice !== "custom";
            const disabled = local.coach_voice == null;
            return (
              <div key={key} className={[SURFACE_INSET, "px-3 py-2", disabled && "opacity-50"].join(" ")}>
                <div className="flex items-center justify-between">
                  <div className="text-sm capitalize">{key}</div>
                  <div className="text-sm tabular-nums opacity-80">{clamp01(v)}%</div>
                </div>
                <input
                  ...
                  className={["w-full mt-2", (locked || disabled) ? "opacity-50 cursor-not-allowed" : "opacity-100", "accent-emerald-500"].join(" ")}
                />
              </div>
            );
          })}
      ...
      <section className={SECTION}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const filtered = (local.secondary_mix ?? []).filter(s => s.sport !== v);
          ...
          <div className="sm:col-span-2">
            ...
            <div className={[SURFACE_INLINE, "px-3 py-2 text-sm font-semibold tabular-nums", shareWarn ? "text-rose-300" : "opacity-90"].join(" ")}>
              ...
              <div key={sec.sport} className={[SURFACE_INLINE, "px-3 py-2"].join(" ")}>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <div className="inline-flex items-center gap-1">
                    {(["none","supplement","improve"] as SecondaryRole[]).map(r => {
                      const active = sec.role === r;
                      return (
                        <button
                          key={r}
                          type="button"
                          onClick={() => updateSecondary(sec.sport, { role: r, share_pct: r === "none" ? 0 : sec.share_pct })}
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-[160px]">
                    <input
                      ...
                      className={["flex-1", "accent-emerald-500", disableSlider ? "opacity-50 cursor-not-allowed" : ""].join(" ")}
                    />
                  ...
                  <div className="ml-auto flex gap-2">
                    <button
                      type="button"
                      onClick={() => updateSecondary(sec.sport, { role: "none", share_pct: 0 })}
                    ...
                    <button
                      type="button"
                      onClick={() => updateSecondary(sec.sport, { role: "supplement", share_pct: 25 })}
                      className={[PILL_BUTTON, "text-xs px-2 py-1"].join(" ")}
                      title="Reset 25%"
                    >
                      reset
        ...
        <div className={FORM_GRID_TWO}>
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(e) => setLocal(prev => ({ ...prev, preferences: { ...prefDefaults(prev), avoid_back_to_back_hard: e.target.checked } }))}
              onInput={markDirty}
            />
          ...
          <div className={FORM_GRID_SPLIT}>
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) => setLocal(prev => ({ ...prev, preferences: { ...prefDefaults(prev), use_zones: e.target.checked } }))}
                onInput={markDirty}
              />
              Use zones
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) => setLocal(prev => ({ ...prev, preferences: { ...prefDefaults(prev), wu_cd_detail: e.target.checked } }))}
                onInput={markDirty}
              />
      ...
      {/* ADVANCED TOGGLE */}
      <div className="flex">
        <button type="button" onClick={() => setShowAdv(s=>!s)} className={[PILL_BUTTON, "mx-auto"].join(" ")} aria-expanded={showAdv}>
      ...
      {showAdv && (
        <>
          ...
          <section className={SECTION}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!(local as any).vo2max_training} onChange={(e) => setPref("vo2max_training" as any, e.target.checked as any)} />
                Include VO₂max blocks (run)
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!(local as any).ftp_training} onChange={(e) => setPref("ftp_training" as any, e.target.checked as any)} />
                Include FTP blocks (ride)
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!(local as any).threshold_focus} onChange={(e) => setPref("threshold_focus" as any, e.target.checked as any)} />
                Threshold focus (more Z3/Z4)
              </label>
            </div>

            <div className="mt-3 flex flex-wrap gap-2">
              <button className={[PILL_BUTTON, (local as any).polarized_model ? ACTIVE_PILL : "border-white/15"].join(" ")} onClick={() => setPref("polarized_model" as any, true as any)}>
                Polarized (80/20)
              </button>
              <button className={[PILL_BUTTON, (local as any).pyramidal_model ? ACTIVE_PILL : "border-white/15"].join(" ")} onClick={() => setPref("pyramidal_model" as any, true as any)}>
                Pyramidal
              </button>
              <button className={PILL_BUTTON} onClick={() => setLocal(p => ({ ...p, polarized_model: false, pyramidal_model: false } as any))}>
                Clear model
              </button>
            </div>
          </section>
          ...
          <section className={SECTION}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              <select className={inputClass} value={extDraft.day} onChange={(e)=>setExtDraft(d=>({ ...d, day: e.target.value as DayAbbrev }))}>
                {ALL_DAYS.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
              <select className={inputClass} value={extDraft.sport} onChange={(e)=>setExtDraft(d=>({ ...d, sport: e.target.value as ExternalSport }))}>
                {EXT_SPORTS.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              <select className={inputClass} value={extDraft.intensity} onChange={(e)=>setExtDraft(d=>({ ...d, intensity: e.target.value as ExternalIntensity }))}>
                {EXT_INTENS.map(i => <option key={i} value={i}>{i}</option>)}
              </select>
              <TextField placeholder="note (optional)" value={extDraft.note ?? ""} onChange={(e)=>setExtDraft(d=>({ ...d, note: (e.target as HTMLInputElement).value }))} />
            </div>
            <div className="mt-2"><Button onClick={()=>{
              ...
              markDirty();
            }} size="sm" variant="success">Add external</Button></div>
              ...
              <ul className="mt-3 space-y-2">
                {((local as any).external_activities ?? []).map((a: any, idx: number) => (
                  <li key={idx} className={[SURFACE_INLINE, "px-3 py-2 flex items-center justify-between"].join(" ")}>
                    <span className="text-sm">{a.day} · {a.sport} · {a.intensity}{a.note ? ` — ${a.note}` : ""}</span>
          ...
          <section className={SECTION}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              <select className={inputClass} value={injDraft.area} onChange={(e)=>setInjDraft(d=>({ ...d, area: e.target.value as InjuryArea }))}>
                {INJ_AREAS.map(a => <option key={a} value={a}>{a}</option>)}
              </select>
              <select className={inputClass} value={injDraft.type} onChange={(e)=>setInjDraft(d=>({ ...d, type: e.target.value as InjuryType }))}>
                {INJ_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <TextField placeholder="note (e.g., foot pain…)" value={injDraft.note ?? ""} onChange={(e)=>setInjDraft(d=>({ ...d, note: (e.target as HTMLInputElement).value }))} containerClassName="md:col-span-2" />
            </div>
            <div className="mt-2"><Button onClick={()=>{
              const cur = (local as any).injuries ?? [];
              setLocal(p => ({ ...p, injuries: [...cur, { ...injDraft, note: injDraft.note?.trim() || undefined }] } as any));
              markDirty();
            }} size="sm" variant="success">Add injury</Button></div>

            {((local as any).injuries ?? []).length > 0 && (
              <ul className="mt-3 space-y-2">
                {((local as any).injuries ?? []).map((it: any, idx: number) => (
                  <li key={idx} className={[SURFACE_INLINE, "px-3 py-2 flex items-center justify-between"].join(" ")}>
                    <span className="text-sm">{it.area} · {it.type}{it.note ? ` — ${it.note}` : ""}</span>
                    <Button size="sm" variant="danger" onClick={()=>{
                      const cur = (local as any).injuries ?? [];
          ...
          <section className={SECTION}>
            ...
            <div className="flex flex-wrap gap-2">
              {FOCUS_CHOICES.map(k => {
                ...
                return (
                  <button key={k} onClick={() => { setPref("focus_areas" as any, next as any); }} className={[PILL_BUTTON, active ? ACTIVE_PILL : "border-white/15"].join(" ")}>
                    {k}
                  </button>
                );
              })}
            ...
            <div className="flex flex-wrap gap-2">
              {AVOID_CHOICES.map(k => {
                ...
                const next = toggleInArray(cur, k);
                return (
                  <button key={k} onClick={() => { setPref("avoid_zones" as any, next as any); }} className={[PILL_BUTTON, active ? ACTIVE_PILL : "border-white/15"].join(" ")}>
                    {k}
                  </button>
                );
              })}
          ...
          <section className={SECTION}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!(local as any).rehab_focus?.stretching}
                  onChange={(e)=> setPref("rehab_focus" as any, { stretching: e.target.checked, mobility: !!(local as any).rehab_focus?.mobility, balance: !!(local as any).rehab_focus?.balance, recovery_protocol: (local as any).rehab_focus?.recovery_protocol ?? null } as RehabFocus)} />
                Stretching
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!(local as any).rehab_focus?.mobility}
                  onChange={(e)=> setPref("rehab_focus" as any, { stretching: !!(local as any).rehab_focus?.stretching, mobility: e.target.checked, balance: !!(local as any).rehab_focus?.balance, recovery_protocol: (local as any).rehab_focus?.recovery_protocol ?? null } as RehabFocus)} />
                Mobility
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!(local as any).rehab_focus?.balance}
                  onChange={(e)=> setPref("rehab_focus" as any, { stretching: !!(local as any).rehab_focus?.stretching, mobility: !!(local as any).rehab_focus?.mobility, balance: e.target.checked, recovery_protocol: (local as any).rehab_focus?.recovery_protocol ?? null } as RehabFocus)} />
                Balance/Proprioception
              </label>
              <TextField placeholder="protocol key (optional)" value={(local as any).rehab_focus?.recovery_protocol ?? ""}
                onChange={(e)=> setPref("rehab_focus" as any, { stretching: !!(local as any).rehab_focus?.stretching, mobility: !!(local as any).rehab_focus?.mobility, balance: !!(local as any).rehab_focus?.balance, recovery_protocol: (e.target as HTMLInputElement).value || null } as RehabFocus)} />
            </div>
          </section>
        </>
      )}
