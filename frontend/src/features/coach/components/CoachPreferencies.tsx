...
export default function PrefsForm() {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const ms =
        (local.main_sport as any) || local.primary_sports?.[0] || "run";
  ...
  const onRefresh = async () => {
    if (!userId) return;
    try {
      const fresh = (await refreshCoachPrefsFromDB(
        userId
  ...
  const mainSport = useMemo<SportKind>(
    () => (local.main_sport as SportKind) || "run",
    [local.main_sport]
  );
  ...
  const secondary = useMemo<SecondaryMix[]>(() => {
    ...
    const missing = ALL_SPORTS.filter((s) => s !== mainSport)
  ...
  const removeExternal = (idx: number) => {
    ...
    setLocal(
      (p) =>
        ({
          ...p,
          external_activities: cur.filter((_: any, i: number) => i !== idx),
        } as any)
    );
  };
  ...
  const removeInjury = (idx: number) => {
    ...
    setLocal(
      (p) =>
        ({
          ...p,
          injuries: cur.filter((_: any, i: number) => i !== idx),
        } as any)
    );
  };
  ...
  const personas: { key: CoachPersona; label: string; hint: string }[] = [
    {
      key: "oldschooler",
      label: "Oldschooler",
      hint: "Prísny drill, direktívny tón, minimum emócií.",
    },
    {
      key: "motivator",
      label: "Motivator",
      hint: "Podporný, pozitívny, občas emoji.",
    },
    {
      key: "analyst",
      label: "Štatistik",
      hint: "Vecný, analytický, čísla > dojmy.",
    },
    {
      key: "realist",
      label: "Realista",
      hint: "Úprimný challenge, nikdy toxický.",
    },
  ];
      ...
      <section className={SECTION}>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-xs opacity-70">
            Preset → zamkne slidery · Custom → odomkne
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {(
            [
              { key: "oldschooler", label: "Oldschooler" },
              { key: "motivator", label: "Motivator" },
              { key: "analyst", label: "Analyst" },
              { key: "realist", label: "Realist" },
              { key: "custom", label: "Custom" },
            ] as { key: CoachPersona; label: string }[]
          ).map(({ key, label }) => {
            const active = (local.coach_voice as CoachPersona) === key;
            return (
              <button
                key={key}
                type="button"
                onClick={() => {
                  // ak preset (nie custom), nastav defaultové hodnoty a zamkni slidery
                  if (key !== "custom") {
                    const tone =
                      PERSONA_TONES[key as Exclude<CoachPersona, "custom">];
                    setPref("coach_voice", key);
                    setPref("coach_tone", { ...tone });
                  } else {
                    // odomknúť slidery, ponechať aktuálne hodnoty
                    setPref("coach_voice", "custom");
                    // ak by chýbali hodnoty, doplň sane defaults
                    const t = local.coach_tone ?? {
                      directness: 50,
                      praise: 50,
                      challenge: 50,
                      emoji: 50,
        ...
        <div className="mt-3 grid grid-cols-1 gap-2">
          {(
            ...
            const v = Number(local.coach_tone?.[key] ?? 50);
            const locked = (local.coach_voice as CoachPersona) !== "custom";
            return (
              <div key={key} className={[SURFACE_INSET, "px-3 py-2"].join(" ")}>
                  ...
                  onChange={(e) => {
                    ...
                    setPref("coach_tone", {
                      directness:
                        key === "directness"
                          ? nv
                          : clamp01(Number(local.coach_tone?.directness ?? 50)),
                      praise:
                        key === "praise"
                          ? nv
                          : clamp01(Number(local.coach_tone?.praise ?? 50)),
                      challenge:
                        key === "challenge"
                          ? nv
                          : clamp01(Number(local.coach_tone?.challenge ?? 50)),
                      emoji:
                        key === "emoji"
                          ? nv
                          : clamp01(Number(local.coach_tone?.emoji ?? 20)),
                      explain:
                        key === "explain"
                          ? nv
                          : clamp01(Number(local.coach_tone?.explain ?? 60)),
                    });
                  }}
                  className={[
                    "w-full mt-2",
      ...
      {showAdv && (
        <>
          ...
          <section className={SECTION}>
            ...
            <div className="mt-2">
              <Button onClick={addExternal} size="sm" variant="success">
                Add external
              </Button>
            </div>
          ...
          <section className={SECTION}>
            ...
            <div className="mt-2">
              <Button onClick={addInjury} size="sm" variant="success">
                Add injury
              </Button>
            </div>
