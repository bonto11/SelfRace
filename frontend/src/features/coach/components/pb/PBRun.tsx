...
import {
  distanceOptions, distanceLabel, getBests, saveBest, deleteBest, type UserBest,
      ...
      <div className="grid gap-2 sm:grid-cols-12 items-start">
        ...
        <div className="relative sm:col-span-2 w-full max-w-[180px]">
          <div className="bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-center select-none truncate">
            {form.achieved_at ? form.achieved_at.replaceAll("-", ".") : "YYYY-MM-DD"}
          </div>
          <input
            type="date"
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-1 text-xs opacity-75">
                  ...
                  {(b as any).activity_name ? <> · <span className="opacity-90">{(b as any).activity_name}</span></> : null}
                </div>
              </div>
            </div>
          </SwipeRow>
...
function SwipeRow({
  ...
  const commit = () => setTx(Math.abs(tx) > Math.abs(SNAP_OPEN) / 2 ? SNAP_OPEN : SNAP_CLOSED);
  ...
  return (
    <li
      ...
      onTouchMove={(e) => {
        ...
        // len doľava
        setTx(Math.max(-200, Math.min(0, dx + (tx === SNAP_OPEN ? SNAP_OPEN : 0))));
      }}
      onTouchEnd={() => { startX.current = null; commit(); }}
    >
      {/* actions */}
