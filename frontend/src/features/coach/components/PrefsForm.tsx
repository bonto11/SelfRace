...
export default function PrefsForm({
  ...
  return (
    <div className="bg-gray-800 rounded p-3 space-y-3">
      ...
      {/* Long run days */}
      <div className="space-y-1">
        <div className="text-sm opacity-80">Preferred long-run days</div>
        {(["Sat","Sun","Fri","Mon","Tue","Wed","Thu"] as DayAbbrev[]).map((d) => (
          <button
            key={d}
            type="button"
            onClick={() =>
              onChange({
                preferences: {
                  ...withPrefs(v.preferences),
                  long_run_days: toggleDay(v.preferences?.long_run_days, d),
                },
              })
            }
            className={`mr-1 mb-1 px-2 py-1 rounded border text-xs ${
              (v.preferences?.long_run_days ?? []).includes(d)
