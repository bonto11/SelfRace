...
import { distanceOptions, distanceLabel, getBests, saveBest, deleteBest, type UserBest } from "@/shared/api/bests";
...
type Form = { distance_m: string; time_str: string; activity_id: string; achieved_at: string; };
...
export default function PBRun() {
  ...
  const refresh = async () => {
    if (!userId) return;
    setLoading(true);
    try { setRows(await getBests(userId, "run")); } catch (e: any) { error(String(e?.message ?? e)); }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs opacity-80">
        Favorite distance: <strong>{distanceLabel(favM, "run")}</strong>
      </div>
      ...
      <div className="grid grid-cols-[160px_160px_160px_180px_auto] gap-2 items-center">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setForm(EMPTY)} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1.5 rounded text-sm">
            Clear
          </button>
          <button onClick={refresh} disabled={loading} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1.5 rounded text-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={6} className="py-3 opacity-70">No records yet.</td></tr>
              ...
              <tr key={b.distance_m} className="border-t border-gray-700/60">
                <td className="py-2 pr-4">
                  <button
                    ...
                    className={`text-lg leading-none ${favM === b.distance_m ? "text-yellow-400" : "text-gray-500 hover:text-gray-300"}`}
                  >★</button>
                </td>
                ...
                <td className="py-2 pr-2">
                  {pendingDelete === b.distance_m ? (
                    <div className="flex gap-2">
                      <button className="text-xs bg-red-600 hover:bg-red-700 text-white px-2 py-0.5 rounded"
                        onClick={() => confirmDelete(b.distance_m)}>Confirm</button>
                      <button className="text-xs underline opacity-90 hover:opacity-100"
                    ...
                    <div className="flex gap-3">
                      ...
                      <button className="text-xs underline opacity-90 hover:opacity-100"
                        onClick={() => setPendingDelete(b.distance_m)}>Delete</button>
                    </div>
                  )}
                </td>
              </tr>
