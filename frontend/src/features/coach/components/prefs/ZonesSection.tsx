...
type Props = {
  zones: any | undefined;
  /** LTHR (typicky LT2 HR) pre aktuálny šport z Thresholds sekcie */
  lthrBpm?: number | null;
...
function recalc(mode: ZoneCalcMode, z: any, lthrBpm?: number | null) {
  ...
  if (mode === "percent_lthr" && Number.isFinite(Number(lthrBpm))) {
    ...
    out.z1_min = Math.round(L * 0.81);
    out.z1_max = Math.round(L * 0.89);
    out.z2_min = Math.round(L * 0.9);
    out.z2_max = Math.round(L * 0.93);
    out.z3_min = Math.round(L * 0.94);
    out.z3_max = Math.round(L * 0.99);
    out.z4_min = Math.round(L * 1.0);
    out.z4_max = Math.round(L * 1.06);
    out.z5_min = out.z4_max + 1;
    out.z5_max = out.z5_min + 10;
    return out;
  }

  // fallback: bez LTHR nič nemeníme (ponechaj manuálne/aktuálne hodnoty)
  return out;
}
...
export default function ZonesSection({
  ...
  // Recalc pri zmene módu, HRmax aj LTHR
  useEffect(() => {
    if (!zones) return;
    ...
    <section className={SECTION}>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          <InfoPopover text="Výpočet zón: HRmax alebo %LTHR. LTHR sa berie z Thresholds (LT2 HR pre aktívny šport)." />
            ...
            <div className={[SURFACE_INLINE, "px-3 py-2"].join(" ")}>
              <SelectField
                ...
                hint={
                  ...
                  !Number.isFinite(Number(lthrBpm))
                    ? "Zadaj LT2 HR v sekcii Thresholds (running/cycling podľa športu)."
                    : undefined
                }
              />
            </div>
