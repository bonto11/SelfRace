...
const COL = { z1:"#60A5FA", z2:"#34D399", z3:"#FBBF24", z4:"#F97316", z5:"#EF4444", grid:"rgba(255,255,255,.18)" };
...
export default function HrChart({
  xs, ys, height = 120, compact = false,
}: { xs: number[]; ys: (number|null)[]; height?: number; compact?: boolean }) {
  ...
  const Svg = useMemo(() => {
    ...
    for (let i=0;i<n;i++) { const h=ys[i]; if (h!=null) { xv.push(xs[i]); yv.push(h); } }
    if (!yv.length) return () => <div className="opacity-70 text-sm">HR stream nie je k dispozícii.</div>;
    ...
    // mriežka
    const yTicks = 4, yVals = Array.from({length:yTicks+1},(_,i)=>minY + i*(maxY-minY)/yTicks);
    const xTicks = 5, xVals = Array.from({length:xTicks+1},(_,i)=>minX + i*(maxX-minX)/xTicks);
    ...
    for (let i=0;i<5;i++){
      const yTop = sy(levels[i+1]), yBot = sy(levels[i]);
      bands.push(
        <rect key={`band-${i}`} x={padL} width={W-padL-padR}
    ...
    for (let i=1;i<yv.length;i++){
      ...
      segs.push(<line key={`s-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke={col} strokeWidth={compact?1.6:2} strokeLinecap="round"/>);
    }

    // legenda – centrovaná hore
    const LEG_W = 5 * 36; // približná šírka legendy
    const LEG_X = (W - LEG_W)/2, LEG_Y = padT - 4; // trošku bližšie hore
    const Legend = () => (
      <g transform={`translate(${LEG_X},${LEG_Y})`}>
        {[
          ...
          <g key={t} transform={`translate(${i*36},0)`}>
            ...
            <text x={8} y={2} fontSize={10} fill="#cbd5e1">{t}</text>
          </g>
        ))}
      </g>
    );
      ...
      <>
        ...
        {xVals.map((t,i)=>(
          <g key={`gx-${i}`}>
            ...
            <text x={sx(t)} y={H-padB+14} textAnchor="middle" fontSize={10} fill="#cbd5e1">
        ...
        <text
          x={padL+4} y={(padT+(H-padB))/2}
          transform={`rotate(-90 ${padL+4} ${(padT+(H-padB))/2})`}
          textAnchor="middle" fontSize={10} fill="#94a3b8">bpm</text>
        <text
          x={(padL+(W-padR))/2} y={H-4}
          textAnchor="middle" fontSize={10} fill="#94a3b8">čas</text>
      </>
    );
    ...
    return () => (
      <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} role="img" aria-label="HR priebeh">
        ...
        <Axis/>
        <defs>
          <clipPath id="hrClip"><rect x={padL} y={padT} width={W-padL-padR} height={H-padT-padB}/></clipPath>
        </defs>
