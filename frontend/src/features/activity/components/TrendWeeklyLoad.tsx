...
export default function TrendWeeklyLoad({
  ...
  const datasets = useMemo(() => {
    ...
    if (metric === "km") {
      pushBar(
        "Km (run)",
        weeks.map((w) => w.km_run),
        C.run
      );
      pushBar(
        "Km (bike)",
        weeks.map((w) => w.km_ride),
        C.bike
      );
      pushBar(
        "Km (mixed)",
        weeks.map((w) => w.km_mixed),
        C.mixed
      );
      pushBar(
        "Km (skate)",
        weeks.map((w) => w.km_skate),
        C.skate
      );
    } else if (metric === "time") {
      pushBar(
        "Run",
        weeks.map((w) => w.time_run_min),
        C.run
      );
      pushBar(
        "Bike",
        weeks.map((w) => w.time_ride_min),
        C.bike
      );
      pushBar(
        "Strength",
        weeks.map((w) => w.time_strength_min),
        C.strength
      );
      pushBar(
        "Mixed",
        weeks.map((w) => w.time_mixed_min),
        C.mixed
      );
      pushBar(
        "Skate",
        weeks.map((w) => w.time_skate_min),
        C.skate
      );
      pushBar(
        "Other",
        weeks.map((w) => w.time_other_min),
        C.other
      );
    } else {
      ...
      pushBar(
        "TRIMP (run)",
        weeks.map((w) => w.trimp_run),
        C.run
      );
      pushBar(
        "TRIMP (bike)",
        weeks.map((w) => w.trimp_ride),
        C.bike
      );
      pushBar(
        "TRIMP (strength)",
        weeks.map((w) => w.trimp_strength),
        C.strength
      );
      pushBar(
        "TRIMP (mixed)",
        weeks.map((w) => w.trimp_mixed),
        C.mixed
      );
      pushBar(
        "TRIMP (skate)",
        weeks.map((w) => w.trimp_skate),
        C.skate
      );
      pushBar(
        "TRIMP (other)",
        weeks.map((w) => w.trimp_other),
        C.other
      );
    }
