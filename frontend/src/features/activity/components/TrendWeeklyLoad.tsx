...
export default function TrendWeeklyLoad({
  ...
  // Séria & osy (z tvojho pôvodného kódu) – beží bez zmien
  ...
  // MINI / DETAIL režim: vyberieme dáta a upravíme options
  const compact = isPortrait && !showDetail;
  const labels = compact ? labelsAll.slice(-THEME.mobile.miniWeeks) : labelsAll;
  const sliceIdx = labelsAll.length - labels.length;

  const monoSeries = compact
    ? monoSeriesAll.slice(-THEME.mobile.miniWeeks)
    : monoSeriesAll;
  const strainSeries = compact
    ? strainSeriesAll.slice(-THEME.mobile.miniWeeks)
    : strainSeriesAll;

  const slicedWeeks = compact ? weeks.slice(-THEME.mobile.miniWeeks) : weeks;

  // datasets – tvoje pôvodné, len berú `slicedWeeks`
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded shadow relative">
      {/* header riadok (filter + tlačidlá) - nezmenené */}
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* PORTRAIT: prepínač detailného grafu */}
          {isPortrait && (
            <button
      ...
      {/* GRAF – mini vs. detail v scroll kontajneri */}
      {loading ? (
        ...
        <>
          ...
          {/* DETAIL (portrait) – horizontálny scroll */}
          {isPortrait && showDetail && (
