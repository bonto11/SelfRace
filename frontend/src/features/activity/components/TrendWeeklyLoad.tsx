...
export default function TrendWeeklyLoad({
  ...
  useEffect(() => {
    if (mode === "full" && lookback > 8) setLookback(8);
  ...
  const monoSeriesAll = useMemo(
    () => weeks.map((w) => w.monotony?.[metric] ?? null),
    [weeks, metric]
  );
  const strainSeriesAll = useMemo(
    () => weeks.map((w) => w.strain?.[metric] ?? null),
    [weeks, metric]
  );

  const monoMax =
    monoSeriesAll.filter((v): v is number => v != null).length > 0
      ? Math.max(1, ...monoSeriesAll.filter((v): v is number => v != null))
      : 3;
  const strainMax =
    strainSeriesAll.filter((v): v is number => v != null).length > 0
      ? Math.max(1, ...strainSeriesAll.filter((v): v is number => v != null))
      : 10;
  ...
  // >>> ZMENA: bar options pod `datasets.bar` (nie `elements.bar`)
      ...
      tooltip: {
        callbacks: {
          label: (ctx) => {
            ...
            if (!compact && ctx.dataset.yAxisID === "y1")
              return `${label}: ${v?.toFixed?.(2) ?? v}`;
            if (!compact && ctx.dataset.yAxisID === "y2")
  ...
  const perWeekPx = 24;
  const scrollMinWidth = Math.max(
    THEME.layout.tableMinWidth,
    perWeekPx * labels.length + 120
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs">
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sRun} onChange={(e) => setSRun(e.target.checked)} /> Run
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sBike} onChange={(e) => setSBike(e.target.checked)} /> Bike
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sStrength} onChange={(e) => setSStrength(e.target.checked)} /> Strength
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sMixed} onChange={(e) => setSMixed(e.target.checked)} /> Mixed
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sSkate} onChange={(e) => setSSkate(e.target.checked)} /> Skate
            </label>
            <label className="flex items-center gap-1">
              <input type="checkbox" checked={sOther} onChange={(e) => setSOther(e.target.checked)} /> Other
            </label>
          </div>
