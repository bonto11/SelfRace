...
const PX_PER_LABEL = 26; // šírka na jeden týždeň – ako vo widgete
...
export default function TrendWeeklyLoad({ onPickWeek }: { onPickWeek?: (w: WeekPick) => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setWeeks(raw.map((w) => ({
        ...
        trimp_skate: num(w.trimp_skate), trimp_other: num(w.trimp_other ?? w.other_trimp),
  ...
  const options: ChartOptions<"bar"|"line"> = {
    ...
    // rovnaké nastavenie barov ako vo widgete
    ...
    scales:{
      ...
      y:{
        beginAtZero:true,
        title:{ display:true, text: metric==="km"?"km": metric==="time"?"min":"TRIMP", color:"#cbd5e1" },
        ticks:{ color:"#cbd5e1" },
      ...
      y1:{
        position:"right",
        min:0, max:Math.max(3, Math.ceil(monoMax+0.5)),
      ...
      y2:{
        position:"right",
        min:0, max: Math.ceil(strainMax*1.1),
        grid:{ drawOnChartArea:false },
  ...
  // ----- RENDER (NEPRETEČIE) -----
  const minWidth = Math.max(labels.length * PX_PER_LABEL, 320);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded shadow relative">
      ...
      {/* scroller: šírku vnútra viažeme na počet labelov */}
      <div className="overflow-x-auto">
        <div style={{ minWidth, paddingBottom: 4 }}>
