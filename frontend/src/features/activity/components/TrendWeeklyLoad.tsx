...
export default function TrendWeeklyLoad({
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await fetch(
        `${API_URL}/analytics/weekly/${userId}?weeks=${lookback}`
      );
  ...
  // pravé osi – dynamicky podľa dát, aby krivky nikdy “neodseklo”
  ...
  const options = useMemo(
    () =>
      buildWeeklyOptions(metric, monoMax, strainMax, {
  ...
  // KĽÚČOVÉ: šírka obsahu pre horizontálny scroll vnútri karty
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded shadow relative max-w-full">
      ...
      {/* Scrolluje sa LEN vnútri karty */}
      <div className="overflow-x-auto overflow-y-hidden rounded-md chart-fixed-h w-full">
