...
export default function ActivityDetail({ activityId }: Props) {
  ...
  // zamkneme scroll pozadia pri fullscreene
  useEffect(() => {
    if (!showFull) return;
  ...
  return (
    <div className={`${CARD} space-y-2`}>
      ...
      {/* HR priebeh – mini verzia cez celú šírku karty */}
      <div className="mt-3">
        ...
        {streams.time_s.length ? (
          <>
            <div className="mt-2">
              <HrChart xs={streams.time_s} ys={streams.hr} compact bands legend={false} />
            </div>
            {/* drobná legenda pod nadpisom, v jednom riadku */}
            <div className="mt-2">
              <HrChart xs={[0,1]} ys={[0,1]} height={0} compact bands={false} legend />
            </div>
          </>
        ) : (
      ...
      {/* FULLSCREEN OVERLAY (bez vnútorných kariet) */}
      {showFull && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm">
          <div className="absolute inset-0 p-3 md:p-6 overflow-auto overscroll-contain">
            <div className="w-full max-w-none bg-gray-800 rounded-lg shadow-lg mx-auto">
              ...
              {/* na výšku ~80–85vh; na šírku tiež vysoké */}
              <div className="px-3 pb-4">
                <div className="w-full" style={{
                  height: typeof window !== "undefined" &&
                          window.innerHeight > window.innerWidth ? "82vh" : "85vh"
                }}>
                  <HrChart
                    xs={streams.time_s}
                    ys={streams.hr}
                    bands
                    legend
                    // výšku nechá komponent, počíta sa z kontajnera
                  />
                </div>
              </div>
            </div>
