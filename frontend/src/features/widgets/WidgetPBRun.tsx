...
function Row({d}:{d:PBRun}) {
  const label = d.distanceKm === 21.1 ? "Half" : d.distanceKm === 42.2 ? "Marathon" : `${d.distanceKm} km`;
  return (
...
export default function CoachPBRunWidget({ onOpenDetail }:{ onOpenDetail?:()=>void }) {
  return (
    <OpenerWidget title="Coach AI — PB (Running)" accent="bg-indigo-600" onOpenDetail={onOpenDetail}>
      <div className="space-y-1">
        {PB_MOCK.map(pb => <Row key={pb.distanceKm} d={pb} />)}
      </div>
