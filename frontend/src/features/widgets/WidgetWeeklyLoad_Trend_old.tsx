...
export default function WeeklyLoadWidget({
  ...
  onOpenDetail,
}: { title?: string; onOpenDetail?: () => void }) {
  ...
  return (
    <OpenerWidget title={title} accent="bg-blue-700" onOpenDetail={onOpenDetail}>
      {loading ? <div className="opacity-70 text-sm">Načítavam…</div> : <MiniChart data={data} options={options} />}
    </OpenerWidget>
  );
}