...
export default function WeeklyLoadWidget({
  ...
  const datasets = useMemo(() => {
    ...
    push(
      "Run",
      get((w) => w.time_run_min),
      C.run
    );
    if (W.some((w) => w.time_ride_min > 0))
      push(
        "Bike",
        get((w) => w.time_ride_min),
        C.bike
      );
    if (W.some((w) => w.time_strength_min > 0))
      push(
        "Strength",
        get((w) => w.time_strength_min),
        C.strength
      );
    if (W.some((w) => w.time_mixed_min > 0))
      push(
        "Mixed",
        get((w) => w.time_mixed_min),
        C.mixed
      );
    if (W.some((w) => w.time_skate_min > 0))
      push(
        "Skate",
        get((w) => w.time_skate_min),
        C.skate
      );
    if (W.some((w) => w.time_other_min > 0))
      push(
        "Other",
        get((w) => w.time_other_min),
        C.other
      );
