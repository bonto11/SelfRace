    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "20px 0 12px" }}>
      <svg width={128} height={128} viewBox="0 0 128 128">
        ...
        <text x={64} y={82} textAnchor="middle"
          fill={appColors.textMuted} fontSize={11}>
          / 100
        </text>
      </svg>
      {/* Label */}
      <span style={{ fontSize: 18, fontWeight: 700, color: score !== null ? color : appColors.textMuted, marginTop: 4 }}>
        {label}
      </span>
    </div>
  );
...
function ComponentRow({ title, value, sub, score, note, noteColor }: ComponentRowProps) {
  const color = readinessColor(score);
  const pct   = score ?? 0;
...
export default function ReadinessPage() {
  ...
  if (loading) {
    return (
      <PageShell title={t("readiness.detail.title") || "Pripravenosť na tréning"} showBack>
  ...
  const sleepNote = c.sleep.today !== null
    ...
    : c.sleep.today > 540 ? (t("readiness.detail.sleepHigh") || "Viac ako 9 hodín")
    : undefined
    : t("readiness.detail.noData") || "Dáta chýbajú";

  const factorsNotes = [
    c.factors.alcohol  && (t("readiness.detail.factorAlcohol")  || "🍷 Alkohol"),
    c.factors.caffeine && (t("readiness.detail.factorCaffeine") || "☕ Kofeín neskoro"),
    c.factors.food     && (t("readiness.detail.factorFood")     || "🍔 Ťažké jedlo"),
  ].filter(Boolean).join(" · ");
    ...
    >
      ...
      <section className={CARD} style={SURFACE_CARD_STYLE}>
        ...
        {!result.hasEnough && (
          <p style={{ textAlign: "center", fontSize: 13, color: appColors.textMuted, padding: "0 16px 16px" }}>
            {t("readiness.detail.notEnoughData") || "Chýba HRV alebo RHR — zadaj ranné merania"}
          </p>
        )}
      </section>
      ...
      <section className={CARD} style={{ ...SURFACE_CARD_STYLE, marginTop: 12 }}>
        ...
        <ComponentRow
          ...
          note={factorsNotes || (t("readiness.detail.noFactors") || "Žiadne negatívne faktory")}
          noteColor={factorsNotes ? appColors.stateWarning : undefined}
        />
      </section>
    </PageShell>
  );
