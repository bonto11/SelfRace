...
/* ============================================================ */
/* KG-BASED BAR (Total Body Water - ma vlastny min~max rozsah v litroch) */
/* ============================================================ */
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
        ...
        <span style={{ fontSize: 10, color: appColors.textMuted }}>
          {realMin}–{realMax} ({t("bodyScan.average")})
        </span>
...
/* ============================================================ */
/* PERCENT-BASED BAR (Weight/SMM/Body Fat Mass v Muscle-Fat Analysis - */
/* kazdy ma VLASTNU skalu, 100% = stred/priemer na tej konkretnej skale) */
/* ============================================================ */

function PercentScaleBar({
  label,
  displayValue,
  displayUnit,
  percent,
  ...
  const t = useT();
  if (percent == null || scaleMin == null || scaleMax == null) return null;

  const realMin = Math.min(scaleMin, scaleMax);
  const realMax = Math.max(scaleMin, scaleMax);
  const clamped = Math.min(realMax, Math.max(realMin, percent));
  const pctPos = ((clamped - realMin) / (realMax - realMin)) * 100;

  // "Normal" pásmo interpretujeme ako 90-110% (blízko stredu ich vlastnej
  // skaly) - InBody samo o sebe nekreslí explicitné pásmo, len jednu čiaru.
  const statusEn: "Under" | "Normal" | "Over" =
    percent < 90 ? "Under" : percent > 110 ? "Over" : "Normal";
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
        <span style={{ fontSize: 10, color: appColors.textMuted }}>{realMin}%</span>
        <span style={{ fontSize: 10, color: appColors.textMuted }}>100%</span>
        <span style={{ fontSize: 10, color: appColors.textMuted }}>{realMax}%</span>
      </div>
    </div>
  );
}
...
/* ============================================================ */
/* RESEARCH PARAMETERS (BMR, Obesity Degree, SMI) - jednoduche riadky */
/* ============================================================ */

function ResearchParamsRow({
  bmr,
  obesityDegree,
  smi,
}: {
  ...
  const t = useT();
  if (bmr == null && obesityDegree == null && smi == null) return null;

  const items: { label: string; value: string }[] = [];
  if (bmr != null) items.push({ label: t("bodyScan.fields.bmr"), value: `${bmr} kcal` });
  if (obesityDegree != null)
    items.push({ label: t("bodyScan.fields.obesityDegree"), value: `${obesityDegree}%` });
  if (smi != null) items.push({ label: t("bodyScan.fields.smi"), value: `${smi}` });
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 8, marginBottom: 14 }}>
      {items.map((item) => (
        ...
        >
          <span style={{ fontSize: 12, color: appColors.textMuted }}>{item.label}</span>
          <span style={{ fontSize: 12, fontWeight: 700, color: appColors.textPrimary }}>
            {item.value}
          </span>
        </div>
...
/* ============================================================ */
/* PANÁČIK (segmentálna analýza) - ruky vodorovne, popisky pri stred */
/* ============================================================ */
  ...
  const renderLabel = (
    ...
    return (
      <g>
        ...
        <text x={x} y={y + 13} fontSize="13" fontWeight={800} textAnchor={anchor} fill={color}>
          {seg.kg != null ? `${seg.kg}kg` : "—"}
        </text>
        {statusKey && (
          <text x={x} y={y + 26} fontSize="9" fontWeight={600} textAnchor={anchor} fill={color}>
            {t(statusKey as any)}
          </text>
        )}
      </g>
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", padding: "8px 0" }}>
      <svg viewBox="0 0 260 220" width="260" height="220">
        ...
        <rect x="112" y="42" width="36" height="62" rx="10" fill={appColors.surfaceCardBorder} />
        <rect x="60" y="48" width="50" height="14" rx="7" fill={appColors.surfaceCardBorder} />
        <rect x="150" y="48" width="50" height="14" rx="7" fill={appColors.surfaceCardBorder} />
        <rect x="113" y="106" width="15" height="90" rx="7" fill={appColors.surfaceCardBorder} />
        <rect x="132" y="106" width="15" height="90" rx="7" fill={appColors.surfaceCardBorder} />

        {renderLabel("left_arm", "bodyScan.segments.leftArm", 55, 60, "middle")}
        {renderLabel("right_arm", "bodyScan.segments.rightArm", 205, 60, "middle")}
        {renderLabel("trunk", "bodyScan.segments.trunk", 130, 70, "middle")}
        {renderLabel("left_leg", "bodyScan.segments.leftLeg", 75, 165, "middle")}
        {renderLabel("right_leg", "bodyScan.segments.rightLeg", 185, 165, "middle")}
      </svg>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div style={{ marginBottom: 6 }}>
        ...
        >
          {t("bodyScan.sections.bodyComposition")}
        </div>
        <ScaleBar
          label={t("bodyScan.fields.totalBodyWater")}
          value={scan.total_body_water_l}
          normalMin={scan.total_body_water_range_min}
          normalMax={scan.total_body_water_range_max}
          unit=" l"
        />
      </div>
      ...
      <div style={{ marginBottom: 6 }}>
        ...
        <PercentScaleBar
          ...
          displayValue={scan.weight_kg}
          displayUnit=" kg"
          percent={scan.weight_percent}
          scaleMin={scan.weight_scale_min}
          scaleMax={scan.weight_scale_max}
        />
        <PercentScaleBar
          label={t("bodyScan.fields.smmShort")}
          displayValue={scan.skeletal_muscle_mass_kg}
          displayUnit=" kg"
          percent={scan.smm_percent}
          scaleMin={scan.smm_scale_min}
          scaleMax={scan.smm_scale_max}
          direction="higher_better"
        />
        <PercentScaleBar
          label={t("bodyScan.fields.bodyFatMass")}
          displayValue={scan.body_fat_mass_kg}
          displayUnit=" kg"
          percent={scan.body_fat_mass_percent}
          scaleMin={scan.body_fat_mass_scale_min}
          scaleMax={scan.body_fat_mass_scale_max}
        />
      </div>
      ...
      >
        {t("bodyScan.sections.other")}
      </div>
      <ResearchParamsRow
        bmr={scan.basal_metabolic_rate_kcal}
        obesityDegree={scan.obesity_degree_percent}
        smi={scan.smi}
      />
