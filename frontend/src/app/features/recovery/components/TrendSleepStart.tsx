...
function shiftAfterMidnightForChart(dayMin: number): number {
  ...
  return dayMin < 12 * 60 ? dayMin + DAY_MIN : dayMin;
}
    ...
    <div className="p-3 rounded-xl border shadow-xl backdrop-blur-md max-w-xs"
      ...
      {mainData?.value != null ? (
        <div className="flex items-center gap-2 text-sm" style={{ color: mainData.color }}>
          ...
          <span className="font-bold">{minutesToClockLabel(mainData.value, t)}</span>
        </div>
      ) : missingData ? (
...
function ChartInner({ chartData, yMin, yMax, tickInterval, yAxisLabel, COLOR, t, showLegend = true }: ChartInnerProps) {
  ...
  return (
    <>
      ...
      <Tooltip content={<SleepStartTooltip t={t} />}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ flexShrink: 0, paddingTop: 10, paddingBottom: 8 }}>
          <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: "6px 20px", marginBottom: 6 }}>
            {[
              { color: COLOR.main, label: t("recovery.trends.sleepStart.label") },
...
export default function TrendSleepStart() {
  ...
  const missingY = useMemo(() => {
    ...
    let last = -1;
    for (let i = n - 1; i >= 0; i--) { if (Number.isFinite(startMin[i])) last = i; nxt[i] = last; }
    let prev = -1;
    for (let i = 0; i < n; i++) {
      if (Number.isFinite(startMin[i])) { prev = i; continue; }
      const nx = nxt[i];
      if (prev !== -1 && nx !== -1)
        out[i] = (startMin[prev] as number) + ((startMin[nx] as number) - (startMin[prev] as number)) * ((i - prev) / (nx - prev));
      else if (prev !== -1) out[i] = startMin[prev] as number;
      else if (nx  !== -1)  out[i] = startMin[nx]  as number;
    }
    return out;
  }, [startMin]);
  ...
  const yMin = Math.floor(Math.min(...allY) / 60) * 60 - 60;
  const yMax = Math.ceil(Math.max(...allY) / 60) * 60 + 60;
      ...
      <section className={CARD + " relative pb-2"} style={SURFACE_CARD_STYLE}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "14px 16px 10px 16px", gap: 8 }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div className={PANEL_SECTION_TITLE} style={{ color: appColors.textPrimary }}>{t("recovery.trends.sleepStart.title")}</div>
            <div className={PANEL_SECTION_SUBTITLE} style={{ color: appColors.textMuted }}>{t("recovery.trends.sleepStart.subtitle")}</div>
          </div>
