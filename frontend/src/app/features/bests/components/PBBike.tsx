...
export default function PBBike() {
  ...
  const favoriteM = favM ?? 20000; // Default 20km pre Bike
  ...
  const refresh = async () => {
    ...
    setLoading(true);
    try {
      setRows(await apiGetBests(userId, "ride"));
    } catch (e: any) {
  ...
  const handleDelete = async (m: number) => {
    ...
    try {
      await apiDeleteBest(userId, m, "ride");
    ...
    <div className={PANEL_STACK}>
      <div className={PANEL_SECTION}>
        ...
        <div className={PANEL_SECTION_TEXT}>
          <strong>{distanceLabel(favoriteM, "ride")}</strong>
        </div>
      </div>
      ...
      <ul className={PANEL_LIST}>
        {rows
          ...
          .map((b) => {
            ...
            const dist = distanceLabel(b.distance_m, "ride");
