...
export default function PBTriathlon() {
  ...
  const favoriteM = favM ?? 51500; // Default Olympijský triatlon
  ...
  const refresh = async () => {
    ...
    setLoading(true);
    try {
      setRows(await apiGetBests(userId, "triathlon"));
    } catch (e: any) {
  ...
  const distanceSelectOptions = useMemo(() => {
    return [
      ...
      ...distanceOptions("triathlon").map((o) => ({
        value: String(o.m),
        label: o.label,
      })),
    ];
  ...
  const handleDelete = async (m: number) => {
    ...
    try {
      await apiDeleteBest(userId, m, "triathlon");
    ...
    <div className={PANEL_STACK}>
      <div className={PANEL_SECTION}>
        ...
        <div className={PANEL_SECTION_TEXT}>
          <strong>{distanceLabel(favoriteM, "triathlon")}</strong>
        </div>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-12 items-start">
          ...
          <div className="sm:col-span-4">
            <ActivitySelector
              ...
              sports={["triathlon", "mixed", "workout"]}
