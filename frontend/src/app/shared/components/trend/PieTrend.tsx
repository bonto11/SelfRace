...
export function PieTrend({ items, valueFormatter = defaultFormatter, renderCenter, className = "" }: Props) {
  const data = useMemo(() => {
  ...
  return (
    <div className={`flex flex-row items-center gap-6 py-4 justify-center sm:justify-start ${className}`}>
      {/* 1. CHART (SVG Donut) */}
      <div className="relative w-24 h-24 flex-shrink-0">
        <svg viewBox="0 0 42 42" className="w-full h-full transform -rotate-90">
          {/* Background track */}
          <circle cx="21" cy="21" r="15.9155" fill="transparent" stroke="rgba(255,255,255,0.05)" strokeWidth="5" />
          ...
          {data.segments.map((s) => (
            <circle
              ...
              className="transition-all duration-500 ease-out" // Pekná animácia pri načítaní
            />
          ))}
        </svg>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span className="text-[10px] font-bold opacity-80">
            {renderCenter ? renderCenter(data.total) : valueFormatter(data.total)}
          </span>
        </div>
      </div>
        ...
        {data.segments.map((s) => (
          <div key={s.label} className="flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-3">
              <span className="font-mono opacity-90">{valueFormatter(s.val)}</span>
