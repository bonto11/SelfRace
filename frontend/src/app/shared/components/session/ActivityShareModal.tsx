...
// Komponent pre metriku - Flexbox nahradený INLINE-BLOCKOM pre 100% zhodu na iOS
function MetricItem({
  iconName,
  label,
  value,
  ...
  return (
    ...
    >
      ...
      {displayMode !== "icon" && (
        <div
          style={{
            ...
            lineHeight: "10px",
            display: "block",
            // Odsadenie presne na šírku ikony(30) + medzera(10)
            paddingLeft: displayMode !== "text" && !centered ? "40px" : "0px",
          }}
        >
          {label}
        </div>
      ...
      {/* RIADOK: IKONA + HODNOTA + JEDNOTKA */}
      <div style={{ display: "block", lineHeight: "30px", whiteSpace: "nowrap" }}>
        ...
        {displayMode !== "text" && !iconErr && (
          <img
            ...
            style={{
              ...
              verticalAlign: "bottom", // Dokonalé zarovnanie na spodok riadku
              marginRight: "10px",
            }}
        ...
        {/* HODNOTA (Číslo alebo Čas) */}
        <span style={{ display: "inline-block", verticalAlign: "bottom", lineHeight: "28px" }}>
          {typeof value === "string" || typeof value === "number" ? (
             <span style={{ fontSize: "28px", fontWeight: 900, color: textColor, verticalAlign: "bottom", lineHeight: "28px", display: "inline-block" }}>
               {value}
             </span>
          ) : (
             value 
          ...
          {/* JEDNOTKA (z prekladu, menšia veľkosť) */}
          {unit && (
            <span
              style={{
                fontSize: "12px",
                ...
                opacity: 0.5,
                marginLeft: "4px",
                verticalAlign: "bottom", // Zarovnanie na spodok
                lineHeight: "16px",
                paddingBottom: "2px",
                display: "inline-block",
              }}
...
export default function ActivityShareModal({
  ...
  // Funkcia pre vyrenderovanie Času bez Flexboxu (pre 100% kompatibilitu s html2canvas)
  function renderTimeValue(seconds: number | null | undefined, textColor: string) {
    if (!seconds || seconds <= 0) return <span style={{ fontSize: "28px", fontWeight: 900, color: textColor, lineHeight: "28px", verticalAlign: "bottom", display: "inline-block" }}>—</span>;
    const h = Math.floor(seconds / 3600);
    const m = Math.floor((seconds % 3600) / 60);
    const s = Math.round(seconds % 60);

    const valStyle = { fontSize: "28px", fontWeight: 900, color: textColor, verticalAlign: "bottom", lineHeight: "28px", display: "inline-block" };
    const unitStyle = {
      fontSize: "12px",
      fontWeight: "bold",
      color: textColor,
  ...
  useEffect(() => {
    ...
    const generateImage = async () => {
      ...
      try {
        ...
        canvas.toBlob((blob) => {
          if (blob && !isCancelled)
            setReadyFile(new File([blob], "selfrace-training.png", { type: "image/png" }));
          ...
          >
            ...
            <div style={{ padding: "32px", display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <div style={{ textAlign: "center", marginBottom: "32px", width: "100%" }}>
                <h2 style={{ fontSize: "22px", fontWeight: 900, textTransform: "uppercase", color: textColor, margin: 0, lineHeight: 1.2, letterSpacing: "0.02em" }}>
                  {title}
                </h2>
              ...
              {/* NAHRADENIE FLEXBOXU ZA FLOAT: LEFT (100% BEZPEČNÉ PRE SAFARI) */}
                ...
                {activeMetrics.map((item, index) => {
                  ...
                  return (
                    <div
                      key={item.key}
                      style={{
                        float: "left", // Používame float namiesto flex
