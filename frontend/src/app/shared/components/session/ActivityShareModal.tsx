...
export default function ActivityShareModal({ isOpen, onClose, activity, summary }: any) {
  ...
  useEffect(() => {
    if (!mounted) return;
    localStorage.setItem("selfrace_share_prefs", JSON.stringify({ showHr, showPace, showElev, showTime, theme, displayMode }));
  ...
  useEffect(() => {
    ...
    const generateImage = async () => {
      ...
      } catch (err) { if (!isCancelled) setIsGenerating(false); }
    };
    generateImage();
  ...
  const handleShare = async () => {
    if (!readyFile) { toast.error(t("share.generatingWarning")); return; }
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [readyFile] })) {
      try { await navigator.share({ title: t("share.title"), files: [readyFile] }); onClose(); } 
      catch (e: any) { if (e.name !== "AbortError") toast.error(t("share.errorFailed")); }
    } else {
