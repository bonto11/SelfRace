...
export default function DetailActivitiesWrapped() {
  ...
  // 🌟 DEBUG — vidieť, či sa komponent vôbec vyrenderuje a s akým userId
  console.log("[WrappedDetail] RENDER, userId=", userId);
  ...
  const loadStatus = useCallback(async () => {
    // 🌟 DEBUG — či efekt vôbec dobehne sem, a s akým userId
  ...
  useEffect(() => {
    console.log("[WrappedDetail] useEffect fired (loadStatus identity changed)");
    loadStatus();
  }, [loadStatus]);

  const canGenerate = !!status?.can_generate;

  // 🌟 DEBUG — presne to, čo rozhoduje o zamknutí formulára (disabled na inputoch/tlačidle)
  console.log("[WrappedDetail] canGenerate=", canGenerate, "status=", status);
